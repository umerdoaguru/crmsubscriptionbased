...
const UpdateRegistryModal = ({
  ...
  useEffect(() => {
    setRegistry({
      ...registry,
      registry_amount: selected?.registry_amount,
      registry_date: selected?.registry_date,
      registry_notes: selected?.registry_notes,
    });
  }, [selected]);
  ...
  const saveUnitSold = async (e) => {
    ...
    try {
      ...
      const soldRes = await axios.put(
        `https://crm-generalize.dentalguru.software/api/updateRegistry/${selected?.registry_id}`,
        formData,
        {
          headers: {
      ...
      cogoToast.success("Registry updated successfully");
      fetchRegistryDetails();
      onClose();
    } catch (err) {
      console.error(err);
