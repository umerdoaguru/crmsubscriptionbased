  ...
  const fetchQuotations = async () => {
    try {
      const response = await axios.get(
        `https://crm-generalize.dentalguru.software/api/quotation-super-admin/${id}`,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        setQuotationStatus(response.data[0].status);
          ...
          <div id="quotationData" className="QuotationDataPage">
            <div className="container border border-black rounded-lg h-auto m-2 p-4">
              <div className="flex flex-col justify-center items-center">
                <div>
                  <img
                    src="https://one-realty.in/static/media/company_logo.b0c6ab3fa89a853264a3.png"
                    alt="Company logo"
                  />
                </div>
                <div className="flex flex-col justify-center items-center gap-2 text-black font-bold">
                  <h4 className="underline ">
                    First Floor chamber number 1&2 Dutt Residency,opposite
                    stadium
                  </h4>
                  <h4 className="underline ">
                  ...
                  <h4 className="underline ">
                    REGISTRATION NO:- 04/14/01/0060/17 RERA ID NO:-
                    P-JBP-23-4248
                  </h4>
                </div>
                <div className="mt-4">
                ...
                {quotations[0] && (
                  <>
                    <div className="w-50 mr-2">
                      ...
                      {quotations[0]?.customer_name}
                    </div>
                    <div className="flex w-full">
                      <div className="w-50 mr-2">
                        <strong>CONTACT NUMBER:</strong>{" "}
                        {quotations[0]?.contact_number}
                      </div>
                      <div className="w-50 mr-2">
                        <strong>ALTERNATE NUMBER:</strong>{" "}
                        {quotations[0]?.alternate_number}
                      </div>
                    </div>
                    <div className="w-50 mr-2">
                      <strong>ADDRESS:</strong> {quotations[0]?.address}
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2">
                        <strong>ADHAAR NUMBER:</strong>{" "}
                        {quotations[0]?.adhaar_number}
                      </div>
                      <div className="w-50 mr-2">
                    ...
                    <div className="w-50 mr-2">
                      ...
                      {quotations[0]?.project_name}
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2">
                        <strong>UNIT NUMBER:</strong>{" "}
                        {quotations[0]?.unit_number}
                      </div>
                      <div className="w-50 mr-2">
                        <strong>DIMENSION:</strong> {quotations[0]?.dimension}
                      </div>
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2">
                        <strong>RATE:</strong> {quotations[0]?.rate}
                      </div>
                      <div className="w-50 mr-2">
                        <strong>VARIANT:</strong> {quotations[0]?.variant}
                      </div>
                    </div>
                    <div className="w-50 mr-2">
                      <strong>TOTAL DEAL:</strong> {quotations[0]?.total_deal}
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2" v>
                        <strong>BOOKING AMOUNT:</strong>{" "}
                        {quotations[0]?.booking_amount}
                      </div>
                      <div className="w-50 mr-2">
                        <strong> IN WORDS:</strong>{" "}
                        {quotations[0]?.booking_amount_words}
                      </div>
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2">
                        <strong>PAYMENT MODE:</strong>{" "}
                        {quotations[0]?.payment_mode}
                      </div>
                      <div className="w-50 mr-2">
                        <strong>IF FINANCE BANK:</strong>{" "}
                        {quotations[0]?.finance_bank}
                      </div>
                    </div>
                    <div className="w-50 mr-2">
                      <strong>DURATION:</strong> {quotations[0]?.duration}
                    </div>
                    <div className="w-full">
                      <div className="w-50 mr-2">
                        <strong>BALANCE AMOUNT:</strong>{" "}
                        {quotations[0]?.balance_amount}
                      </div>
                      <div className="w-50 mr-2">
                        <strong> IN WORDS:</strong>{" "}
                        {quotations[0]?.balance_amount_words}
                      </div>
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2">
                        <strong>PAYMENT DUE DATE 1:</strong>{" "}
                        {new Date(
                          quotations[0]?.payment_due_date1
                        ).toLocaleDateString()}
                      </div>
                      <div className="w-50 mr-2">
                        <strong>PAYMENT DUE DATE 2:</strong>{" "}
                        {new Date(
                          quotations[0]?.payment_due_date2
                        ).toLocaleDateString()}
                      </div>
                    </div>
                    <div className="w-full flex">
                      <div className="w-50 mr-2">
                        <strong>PAYMENT DUE DATE 3:</strong>{" "}
                        {new Date(
                          quotations[0]?.payment_due_date3
                        ).toLocaleDateString()}
                      </div>
                      <div className="w-50 mr-2">
                        <strong>PAYMENT DUE DATE 4:</strong>{" "}
                        {new Date(
                          quotations[0]?.payment_due_date4
                        ).toLocaleDateString()}
                      </div>
                    </div>
                    <div>
                      <strong>REGISTRY CHARGES:</strong>{" "}
                      {quotations[0]?.registry_charges}
                    </div>
                    <div>
                      <strong>P1 P2 CHARGES:</strong>{" "}
                      {quotations[0]?.p1p2_charges}
                    </div>
                    <div>
                      <strong>REMARKS:</strong> {quotations[0]?.remarks}
                    </div>
                  </>
                )}
              </div>
